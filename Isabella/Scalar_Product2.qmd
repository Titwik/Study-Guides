...
It also has applications in applied mathematics and physics. **Work done** is an important concept in physics and is defined as the amount of energy transferred by an object by an external force when it is moved over a certain distance. 
When the force is constant and in the same direction as the movement of the object, the equation is: $W=|\mathbf{F}||\mathbf{d}|$, that is work done is equal to force times distance. However, if the force is being applied at an angle, such as with a wagon being pulled along by a rope, then you need the component of the force acting in the direction of the motion. The scalar product lets you do this and hence appears in the equation for work done at an angle (see later in the guide for more information).
...
![Geometric interpretation of the Scalar Product, showing the vectors v and u and their difference and a triangle with the corresponding lengths.](../FiguresSP/Figure 1.pdf){#fig-4 width="150%" fig-alt="Two triangles side by side on separate diagrams. The first shows the vectors u and v. u is horizontal and v start from the same place as u and is above. v-u is labelled and is the line joining the 2 vectors lines, making it a triangles. The second shows the same traingle, but instead labels the sides as the corresponding lengths, a,b,c and also labels h,$\psi$ and x."}
:::
::: {.content-hidden when-format="html"}
![Geometric interpretation of the Scalar Product, showing the vectors v and u and their difference and a triangle with the corresponding lengths.](./FiguresSP/Figure1.pdf){#fig-1 fig-alt="Two triangles side by side on separate diagrams. The first shows the vectors u and v. u is horizontal and v start from the same place as u and is above. v-u is labelled and is the line joining the 2 vectors lines, making it a triangle. The second shows the same triangle, but instead labels the sides as the corresponding lengths, a,b,c and also labels h,$\psi$ and x."}
:::
...
$\mathbf{u}\cdot\mathbf{x}=(\mathbf{i}+2\mathbf{j}+\mathbf{k})\cdot(\mathbf{i}+3\mathbf{j}-7\mathbf{k})=(1)(1)+(2)(3)+(1)(-7)=1+6-7=0$
$\mathbf{v}\cdot\mathbf{x}=(4\mathbf{i}-\mathbf{j}+\mathbf{k})\cdot(\mathbf{i}+3\mathbf{j}-7\mathbf{k})=(4)(1)+(-1)(3)+(1)(-7)=4-3-1=0$
:::

:::{.callout-tip}
Note here that the above example can be more easily computed using the **cross product**.
:::

## Applications - Work Done

As stated earlier in the guide, the scalar product lets you calculate the work done by a force at an angle. Recall that the work done when the force vector is in the same direction as the movement of the object is: $W=|\mathbf{F}||\mathbf{d}|$. When a force is applied at an angle, the equation becomes: $W=|\mathbf{F}||\mathbf{d}|\cos(\theta)$, where $\theta$ is the angle between the resultant force vector and the direction of motion. Hence, the work done equation is just the dot product of force and displacement! 
...
where $W$ is the work done, $\theta$ is the  angle between the resultant force vector and the direction of motion, $\mathbf{F}$ is the force vector, and $\mathbf{d}$ is the displacement vector.
:::
...
Suppose you have a wagon that is being pulled along by a force vector of magnitude $8N$ at an angle of $55^\circ$ to the direction of motion and that the wagon is pulled for $50m$. Then the work done by the force can be calculated as follows:
$$W=|\mathbf{F}||\mathbf{d}|\cos(\theta)=(8)(50)(\cos(55^\circ))\approx{229 Nm}$$
:::
